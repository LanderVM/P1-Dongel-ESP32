...
void handleEnergyID()
{
  ...
  case EID_ENROLLED:
    ...
    {
      if (DUE(T_EID_HELLO_FAIL))
        EIDPostHello();
    } // get the webhook url
    else if (DUE(T_EID))
    { // is enrolled and got webhook url
      PostEnergyID();
    }
    if (DUE(T_EID_REFRESH))
      EIDPostHello(); // refresh every 24h
    if (recordNumber != "" && apiAccessToken != "" && !bGotDirective)
      EIDGetDirectives(); // 4.16 feature
    if (bGetPlannerDetails)
      EIDGetDirectDetails();
    if (DUE(T_EID_PLAN))
...
void EIDPostHello()
{
  ...
  */
  ...
  Debug(F("httpResponseCode: "));
  Debugln(httpResponseCode);

  if (httpResponseCode == 200)
  {
    payload = http.getString();
    Debug(F("response body: "));
    Debugln(payload);
    ...
    else
    {
      ...
      else
      {
        ...
        Debug(F("webhookUrl      : "));
        Debugln(eid_webhook);
        Debug(F("authorization   : "));
        Debugln(eid_header_auth);
        Debug(F("x-twin-id       : "));
        Debugln(eid_header_twinid);
        Debug(F("eid_interval_sec: "));
        Debugln(eid_interval_sec);
        Debug(F("recordNumber    : "));
        Debugln(recordNumber);
        Debug(F("apiAccessToken  : "));
        Debugln(apiAccessToken);
...
void EIDGetDirectives()
{
  ...
  if (httpResponseCode == 200)
  {
    ...
    DeserializationError error = deserializeJson(doc, payload);
    if (error)
      DebugTln(F("JSON parsing failed in EIDGetDirectives"));
    else
    {
...
void EIDGetDirectDetails()
{
  ...
  if (EIDDirectiveID.length() == 0)
  {
    DebugTln("No ID present");
...
#ifdef DEBUG
    if (error)
      DebugTln(F("JSON parsing failed in EIDGetDirectDetails"));
    else
    {
      for (int i = 0; i < 14; i++)
      {
        Debugf("signal [%i] : ", i);
        Debugln(StroomPlanData["data"][i]["signal"].as<const char *>());
      }
    }
    JsonEIDplanner();
#endif
...
void EIDGetClaim()
{
  ...
  if (httpServer.arg("action") == "reset")
...
void PostEnergyID()
{
  ...
  */
  if (eid_webhook.length() == 0 || !telegramCount)
    return;
  String payload;
